...
word_to_int, int_to_word, sentence_int, label, length = get_data(metadata_pkl, max_length)
...
class_list = [u"投递质量", u"延误", u"信息质量", u"破损", u"丢失", u"服务态度", u"收寄质量", u"需求建议", u"售后服务"]
...
def print_sentence(sentence_int):
    s = [int_to_word[w] for w in sentence_int if w != 0]
    print(''.join(s))    
...
def test(sess, actor, classifier, RL=True):
    ...
    if RL:
        ...
        for i in range(5):
            print('\nexample:')
            input_i = i
            print_sentence(input_vec[input_i])
            print('\nafter Simplifying:')
            print_sentence(Rinput[input_i])
            print()
...
#
#model = SelfAttRNN()
#classifier.build_graph(sess, model, "/gpu:0")   
#
#actor = ActorNetwork(sess, lstm_dim, optimizer, lr, embeddings)
#
#saver = tf.train.Saver()    
